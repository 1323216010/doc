...
定义：线性表是具有相同数据类型的$n(n ≥ 0)$个数据元素(由数据项组成)的有限序列。
...
定义：数组是由$n(n ≥ 1)$个相同类型的数据元素构成的有限序列
...
树是$n(n ≥ 0)$个结点的有限集。当$n = 0$时，称为空树。任意一棵非空树应满足：
...
二叉树是$n(n ≥ 0)$个结点的有限集，当$n = 0$时，称为空二叉树。任意一棵非空二叉树应满足：
